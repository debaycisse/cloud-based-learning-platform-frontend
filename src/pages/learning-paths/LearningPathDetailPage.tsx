...
import type { LearningPath, Course, CourseProgress } from "../../types"
...
const LearningPathDetailPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchLearningPathDetails = async () => {
      ...
      try {
        ...
        setUserProgress(progressData.course_progress)
      } catch (err) {
  ...
  const isCourseAccessible = (courseIndex: number) => {
    ...
    return previousCourse ? isCourseCompleted(previousCourse._id ? previousCourse._id: "") : false
  }
  ...
  const getOverallProgress = () => {
    ...
    const totalProgress = courses.reduce((sum, course) => sum + getCourseProgress(course._id? course._id: ""), 0)
    return Math.round(totalProgress / courses.length)
  }

  const getCompletedCoursesCount = () => {
    return courses.filter((course) => isCourseCompleted(course._id? course._id: "")).length
  }
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-4">
          {courses.map((course, index) => {
            const progress = getCourseProgress(course._id? course._id: "")
            const isCompleted = isCourseCompleted(course._id? course._id: "")
                      ...
                      <div className="flex items-start justify-between gap-4">
                        ...
                        <div className="flex-shrink-0">
                          {!isAccessible ? (
                            ...
                            <Button variant="outline" size="sm" onClick={() => handleStartCourse(course._id? course._id: "")}>
                              <CheckCircle className="w-4 h-4 mr-2" />
                              Review
                            </Button>
                          ) : isInProgress ? (
                            <Button size="sm" onClick={() => handleContinueCourse(course._id? course._id: "")}>
                              <Play className="w-4 h-4 mr-2" />
                              Continue
                            </Button>
                          ) : (
                            <Button size="sm" onClick={() => handleStartCourse(course._id? course._id: "")}>
                              <Play className="w-4 h-4 mr-2" />
                              Start
                            </Button>
                          )}
